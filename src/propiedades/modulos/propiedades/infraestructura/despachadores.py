...
from propiedades.modulos.propiedades.infraestructura.schema.v1.eventos import EventoPropiedadCreado, EventoPropiedadCreadoFallo, PropiedadCreadoFalloPayload, PropiedadCreadoPayload
...
class Despachador:
    ...
    def publicar_evento(self, evento, topico):
        ...
        if topico == 'eventos-fallo-propiedad':
            payload = PropiedadCreadoFalloPayload(
                fecha_creacion=int(unix_time_millis(evento.fecha_creacion)),
                id_cor = str(evento.id_cor)
            )
            evento_integracion = EventoPropiedadCreadoFallo(data=payload)
            self._publicar_mensaje(evento_integracion, topico, AvroSchema(EventoPropiedadCreadoFallo))
        else:
