...
from sagas.seedwork.dominio.entidades import AgregacionRaiz
...
def registrar_unidad_de_trabajo(serialized_obj):
    from sagas.config.uow import UnidadTrabajoSQLAlchemy
    from flask import session
    if is_flask():
        session['uows'] = serialized_obj
    else:
        with open('uows.pkl', 'wb') as outp:
